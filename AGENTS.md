...
- `src/arch` - RISC-V architecture specifics (`rv32/` subdirectory with struct-based namespaces: `arch.Paging`, `arch.Trap`, `arch.Context`, `arch.Syscall`, `arch.Sbi`)
...
- `src/drivers` - console (printf), VirtIO block driver (SBI lives in `src/arch/rv32/`)
